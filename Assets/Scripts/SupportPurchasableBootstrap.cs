...
public sealed class SupportPurchasableBootstrap : MonoBehaviour
{
    ...
    private static void EnsureRuntimeBootstrap()
    {
        ...
        new GameObject("SupportPurchasableBootstrap").AddComponent<SupportPurchasableBootstrap>();
    }

    private void Start() => ConfigureSupportPurchases();

    public void ConfigureSupportPurchases()
    {
        if (GameManager.Instance == null || GameManager.Instance.sectors == null)
        {
            Debug.LogWarning("Support bootstrap: battlefield GameManager/sectors unavailable; Support purchasables were not configured.");
            return;
        }
        ...
        foreach (Sector sector in GameManager.Instance.sectors)
        {
            ...
            foreach (CapturePoint point in sector.capturePoints)
            {
                ...
                if (attackerTemplate != null && !ContainsSupport(point.attackerPurchasables)) { point.attackerPurchasables = AppendSupport(point.attackerPurchasables, attackerTemplate); attackerAdds++; }
                if (defenderTemplate != null && !ContainsSupport(point.defenderPurchasables)) { point.defenderPurchasables = AppendSupport(point.defenderPurchasables, defenderTemplate); defenderAdds++; }
            }
        }
        if (logConfiguration) Debug.Log($"SUPPORT CONFIG: Added Support purchase option to {attackerAdds} attacker and {defenderAdds} defender catalogs. Cost {supportXPCost} XP.");
    }
    ...
    private PurchasableUnit[] AppendSupport(PurchasableUnit[] existing, GameObject templatePrefab)
    {
        ...
        result.Add(new PurchasableUnit { unitDisplayName = supportDisplayName, unitPrefab = templatePrefab, xpCost = Mathf.Max(0, supportXPCost), unitIcon = null, useExplicitClassification = true, unitCategory = UnitCategory.Infantry, unitClass = UnitClass.Support });
    ...
    private bool ContainsSupport(PurchasableUnit[] units)
    {
        ...
        foreach (PurchasableUnit unit in units)
        {
            ...
            if (unit.TryGetResolvedInfantryClass(out UnitClass unitClass) && unitClass == UnitClass.Support) return true;
        }
        return false;
    }
}
