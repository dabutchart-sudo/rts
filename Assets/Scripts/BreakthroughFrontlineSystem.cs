...
/// <summary>
/// Single source of truth for Breakthrough combat territory.
///
/// Normal play:
/// - Attackers may occupy every sector captured so far plus the active sector.
/// - Defenders may occupy only the active defensive sector.
///
/// Sector transition:
/// - Attackers remain inside the sector they have just captured until the timer expires.
/// - Defenders may use the captured sector plus the next sector while retreating.
///
/// The system also provides a lightweight runtime visualisation so the moving frontline can be
/// observed directly in Game view without adding map-specific scene objects.
/// </summary>
public sealed class BreakthroughFrontlineSystem : MonoBehaviour
{
    ...
    public static bool TryGetAllowedSectorIndices(Faction faction, out List<int> allowedIndices)
    {
        ...
        if (GameManager.Instance.isTransitioningSector)
        {
            ...
            {
                // Attackers clear the captured sector and wait at its forward edge.
                allowedIndices.Add(activeIndex);
            }
            else if (faction == Faction.Defender)
            {
                // Defenders need a legal corridor from the lost sector into the next one.
                allowedIndices.Add(activeIndex);
    ...
    {
        Color attacker = FactionVisuals.AttackerColor;
        Color defender = FactionVisuals.DefenderColor;
        ...
        if (sectorIndex == activeSector)
        {
            // Both factions can fight here, so use a bright neutral frontline marker.
