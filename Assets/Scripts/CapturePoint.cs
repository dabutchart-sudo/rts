...
public class PurchasableUnit
{
    ...
    {
        if (useExplicitClassification) return unitCategory;
        string identityText = GetIdentityText();
        if (ContainsAny(identityText, "tank", "vehicle", "apc", "ifv", "jeep", "truck", "car")) return UnitCategory.Vehicle;
        if (TryInferInfantryClass(identityText, out _)) return UnitCategory.Infantry;
    ...
    {
        if (ContainsAny(value, "engineer")) { resolvedClass = UnitClass.Engineer; return true; }
        if (ContainsAny(value, "recon", "sniper", "scout")) { resolvedClass = UnitClass.Recon; return true; }
        if (ContainsAny(value, "support", "medic")) { resolvedClass = UnitClass.Support; return true; }
        if (ContainsAny(value, "assault", "rifleman")) { resolvedClass = UnitClass.Assault; return true; }
...
public class CapturePoint : MonoBehaviour
{
    ...
    [Range(-100f, 100f)] public float captureProgress = -100f;
    ...
    [Header("Team Colors (runtime uses shared faction palette)")]
    ...
    void Awake()
    {
        attackerColor = FactionVisuals.AttackerColor;
        defenderColor = FactionVisuals.DefenderColor;
        neutralColor = FactionVisuals.NeutralColor;
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Attacker") && !activeAttackers.Contains(other.gameObject)) activeAttackers.Add(other.gameObject);
        else if (other.CompareTag("Defender") && !activeDefenders.Contains(other.gameObject)) activeDefenders.Add(other.gameObject);
    }

    void OnTriggerExit(Collider other)
    {
        if (isLocked) return;
        if (other.CompareTag("Attacker")) activeAttackers.Remove(other.gameObject);
        else if (other.CompareTag("Defender")) activeDefenders.Remove(other.gameObject);
    }
    ...
    void Update()
    {
        ...
        if (isLocked)
        {
            activeAttackers.Clear(); activeDefenders.Clear(); attackerCount = 0; defenderCount = 0; return;
        }
    ...
    {
        if (playerFaction == Faction.Attacker && captureProgress >= 100f) return attackerPurchasables;
        if (playerFaction == Faction.Defender && captureProgress <= -100f) return defenderPurchasables;
