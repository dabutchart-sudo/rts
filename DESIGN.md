...
A saved layout can dress itself with Market pieces: parasols, awnings, and small stalls, kept clear of control points and spawns. A population slider from 1 to 20 sets how many pieces each sector tries to place. Dragging a gold sphere on a piece keeps that piece. The small red sphere deletes it, and that spot stays empty. Dress again replaces only the pieces you have not moved. The population level is saved with the map. Roads, ground materials, other districts, and a palette you drag from are still later. The Market preview stays a look check, not a playable Breakthrough map.
